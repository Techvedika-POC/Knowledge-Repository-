...
export default function KnowledgeQuestPage() {
  ...
  // NOTE: userId in localStorage is used only to decide whether to call "me" endpoint.
  // The server should derive user from JWT; client shouldn't send userId in path/body for non-admins.
  const userId = localStorage.getItem("userId");
  ...
  const handleTopicSelect = async (topicId) => {
    ...
    try {
      // Prefer the "me" endpoint if user is logged in and your api includes auth header.
      // Otherwise fall back to public endpoint.
  ...
  const handleStartTest = async (mod) => {
    ...
    try {
      // map module name to Quiz category (tweak as needed)
      const categoryMap = {
        javascript: "code",
        python: "code",
        sql: "sql",
      ...
      // CALL YOUR BACKEND endpoint (recommended): /api/VLearnTest/generate
      // Backend proxies QuizAPI and returns sanitized questions (no API key in client).
      // Expected format from backend: [{ question, options: [..], correctOption: "option text" }, ...]
