...
export default function IdeathonPage() {
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1 max-w-2xl">
              Browse ideathons — switch between <span className="font-semibold text-sky-600">Current</span>, <span className="font-semibold text-gray-600">Past</span> or <span className="font-semibold text-indigo-600">All</span>. Click an event card to open full details below.
            </p>
          </div>
      ...
      {/* VIEW: detail if selected, else cards */}
      {selectedEvent ? (
        <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} className="w-full">
          <EventDetail
            ...
            onBack={() => { setSelectedEventId(null); setViewMode("current"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
          />
        </motion.div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEvents.map((ev) => (
            <motion.div
              ...
              className="group cursor-pointer text-left p-5 rounded-2xl shadow-sm border transition-all bg-white hover:shadow-lg"
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="ml-2 flex flex-col items-end gap-2">
                  <div className={`text-xs px-2 py-1 rounded-full font-medium ${isCurrentEvent(ev) ? 'bg-sky-50 text-sky-700' : isPastEvent(ev) ? 'bg-gray-100 text-gray-600' : 'bg-amber-50 text-amber-700'}`}>
                    {isCurrentEvent(ev) ? 'Ongoing' : isPastEvent(ev) ? 'Finished' : 'Upcoming'}
                  </div>
              ...
              <div className="mt-4 flex items-center justify-between text-xs text-gray-500">
                {ev.finalSubmissionDeadline ? (
                  <div className="flex items-center gap-2">
                    ...
                    <span>Deadline: <span className="font-medium text-gray-700">{new Date(ev.finalSubmissionDeadline).toLocaleDateString()}</span></span>
                  </div>
                ) : <div /> }
                <div />
              </div>
...
function EventDetail({ event, userId, onPreview, onRegister, onSubmitIdea, fullWidth = false, isFinished = false, onBack }) {
  ...
  useEffect(() => {
    ...
    fetch();
  }, [event.eventId, userId, event.submissions, event.feedbacks]);
  ...
  const trySubmit = () => {
    if (event.finalSubmissionDeadline) {
      ...
      if (new Date() > deadline) {
        ...
        alert("Submissions are closed for this event — the final submission deadline has passed.");
        return;
      }
    }

    onSubmitIdea(event.eventId, isRegistered, event);
  };
  ...
  return (
    <div className={`rounded-2xl p-6 shadow-md border bg-white ${fullWidth ? "" : ""}`}>
      <div className="flex flex-col md:flex-row items-start justify-between gap-4">
        <div className="flex-1">
          <h2 className="text-2xl font-bold text-gray-800">
            {event.title} <span className="text-indigo-600">• {event.category || "Ideathon"}</span>
          </h2>
          ...
          <div className="mt-3 flex items-center gap-3 text-xs text-gray-500">
            {event.finalSubmissionDeadline && (
              <div className="flex items-center gap-1">
                <Calendar className="w-4 h-4 text-gray-400" />
                <span>
                  Deadline: <span className="font-semibold text-gray-700">{new Date(event.finalSubmissionDeadline).toLocaleString()}</span>
                </span>
              </div>
            )}
        ...
        <div className="flex flex-col items-end gap-3">
          ...
          <button onClick={onBack} className="px-4 py-2 rounded-full border bg-white text-sm shadow-sm">← Back to Ideathon Hub</button>
        </div>
      </div>
      ...
      <div className="mt-4">
        {tab === "overview" && (
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-4">Full Timeline</h4>

   
            <div className="space-y-3">
              {timelineEntries.map((t, i) => {
                const isPast = t.date < nowDate;
                const isLast = i === timelineEntries.length - 1;
                const dotColor = isPast ? "bg-gray-400" : "bg-indigo-600";
                return (
                  <div key={t.key} className="flex items-start gap-3">
                    <div className="flex flex-col items-center">
                      <div className={`w-3 h-3 rounded-full ${dotColor}`} />
                      {!isLast && <div className="w-px bg-gray-200 mt-1" style={{ minHeight: 20 }} />}
                    </div>

                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <div className="text-sm font-medium text-gray-800">{t.label}</div>
                        <div className="text-xs text-gray-500">{t.date.toLocaleString()}</div>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="flex gap-3">
          {!isFinished && (
            <>
              ...
              <button onClick={trySubmit} className="px-5 py-2 rounded-full bg-gradient-to-r from-indigo-600 to-sky-500 text-white font-semibold shadow">Submit Your Idea</button>
